    ...
    <>
      ...
      <Col className={styles.userProfile}>
        ...
        <Descriptions title="基本情報" colon={false} bordered column={{ xxl: 4, xl: 3, lg: 3, md: 3, sm: 2, xs: 1 }}>
          <Descriptions.Item label="氏名">{userInfo?.displayname}</Descriptions.Item>
          <Descriptions.Item label="メールアドレス">{userInfo?.email}</Descriptions.Item>
          <Descriptions.Item label="プログラミング歴">{userInfo?.email}</Descriptions.Item>
          <Descriptions.Item label="一言">
            {userInfo?.message ? userInfo?.message : 'よろしくお願いします'}
          </Descriptions.Item>
        </Descriptions>
      </Col>
    </>
